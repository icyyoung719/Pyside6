...
class MyWidget(QWidget):
    ...
    def setupUi(self):    # 创建界面上的控件
        ...
        self.stackedWidget = QStackedWidget()  #建立控件栈
        self.stackedWidget.addWidget(self.widget_1)  #添加第1个窗口
        self.stackedWidget.addWidget(self.widget_2)  #添加第2个窗口

        v = QVBoxLayout(self)  #竖直布局
        v.addWidget(self.stackedWidget)

        self.label_1 = QLabel(self.widget_1)   #在第1个窗口中添加控件
        self.label_1.setGeometry(QRect(120, 40, 230, 30))
        font = QFont("楷体",pointSize=20)
        self.label_1.setFont(font)
        self.label_1.setText("学生考试成绩查询")

        self.label_2 = QLabel(self.widget_1)  #在第1个窗口中添加控件
        self.label_2.setGeometry(QRect(113, 130, 60, 20))
        self.label_2.setText("姓名：")

        self.label_3 = QLabel(self.widget_1)  #在第1个窗口中添加控件
        self.label_3.setGeometry(QRect(100, 160, 70, 20))
        self.label_3.setText("准考证号：")

        self.lineEdit_name = QLineEdit(self.widget_1)  #在第1个窗口中添加控件
        self.lineEdit_name.setGeometry(QRect(190, 130, 113, 20))
        self.lineEdit_name.setClearButtonEnabled(True)  #设置清空按钮
        self.lineEdit_number = QLineEdit(self.widget_1)  #在第1个窗口中添加控件
        ...
        self.btn_enquire = QPushButton(self.widget_1)  #在第1个窗口中添加控件
        self.btn_enquire.setGeometry(QRect(150, 210, 75, 23))
        self.btn_enquire.setText("查询")
        self.btn_enquire.clicked.connect(self.inquire)  #信号与槽的连接

        self.btnClear = QPushButton(self.widget_1)  #在第1个窗口中添加控件
        ...
        self.texBrowser = QTextBrowser()  #定义第2个窗口中的控件
        self.texBrowser.setFont(font)
        v_layout.addWidget(self.texBrowser)
        self.btnReturn = QPushButton("返 回")  #定义第2个窗口中的控件
        v_layout.addWidget(self.btnReturn)
        self.btnReturn.clicked.connect(self.btnReturn_clicked)  #信号与槽的连接
    def inquire(self):
        if self.lineEdit_name.text() != "" and self.lineEdit_number.text() != "":
            ...
            self.stackedWidget.setCurrentWidget(self.widget_2)   #设置当前的窗口
    def btnReturn_clicked(self):
        self.stackedWidget.setCurrentWidget(self.widget_1)  # 设置当前的窗口
